...
nexp = 1000 # TODO: Change this when you have developed your code
...
for i in range(nexp):
    ...
    sig = np.sqrt(np.var(x, ddof=1)) # DONE: adapt for b)
    ...
    ac = xmean - scipy.stats.t.ppf((1-gamma)/2 + gamma, n-1)*sig/np.sqrt(n) # DONE: adapt for c)
    bc = xmean - scipy.stats.t.ppf((1-gamma)/2, n-1)*sig/np.sqrt(n) # DONE: adapt for c)
