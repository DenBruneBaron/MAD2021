...
# Loading data
data = np.loadtxt('../data/men-olympics-100.txt')
N, d = data.shape
print('N = ', N)
print('d = ', d)
...
def muw(X, t, variance, Sigma_0, mu_0, Sigma_w):
    '''
    Compute the posterier mu_w
    '''
    return Sigma_w @ (1/variance * X.T @ t + np.linalg.inv(Sigma_0) @ mu_0)
    
# **Exercise 2d)**
Sigmaw = Sigmaw(X, variance, Sigma0)
muw = muw(X, t, variance, Sigma0, mu0, Sigmaw)
print("muw = " + str(muw))
